...
describe("AuthGuard",()=>{const reflector={getAllAndOverride:jest.fn()};const jwt=new JwtService({secret:"12345678901234567890123456789012"});const db={query:jest.fn().mockResolvedValue({rows:[{role:"fan",status:"ACTIVE"}]})};const guard=new AuthGuard(reflector as any,jwt,db as any);const context=(authorization?:string)=>({getHandler:()=>({}),getClass:()=>({}),switchToHttp:()=>({getRequest:()=>({headers:{authorization},user:undefined})})});beforeEach(()=>reflector.getAllAndOverride.mockReset());
