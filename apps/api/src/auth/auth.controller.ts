...
class RegisterDto{@IsEmail() email!:string;@IsString()@MinLength(2) displayName!:string;@IsString()@MinLength(12) password!:string;@IsIn(["fan","creator"]) role!:Extract<Role,"fan"|"creator">;}
...
export class AuthController{
 ...
 @Public()@Throttle({default:{limit:5,ttl:60000}})@Post("register") register(@Body() dto:RegisterDto){return this.auth.register(dto.email,dto.password,dto.role,dto.displayName);}
