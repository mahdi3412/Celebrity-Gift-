...
class CreateGiftDto{@IsString() creatorId!:string;@IsString() category!:string;@IsBoolean() food?:boolean;@IsBoolean() fragile?:boolean;}
...
export class GiftsController{
 ...
 @Roles("fan","creator","admin","station_staff")@Patch(":id/status") transition(@Req() req:AuthedRequest,@Param("id") id:string,@Body() dto:StatusDto){return this.service.transitionForUser(id,dto.status,req.user.sub,req.user.role as Role);}
}