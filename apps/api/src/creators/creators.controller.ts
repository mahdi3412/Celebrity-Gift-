import {Body,ConflictException,Controller,Get,Param,Patch,Query,Req} from "@nestjs/common";
...
export class CreatorsController{
 ...
 @Roles("creator")@Patch("me") profile(@Req() req:AuthedRequest,@Body() dto:UpdateCreatorDto){return this.service.updateProfile(req.user.sub,dto).catch(error=>{if(error?.code==="23505")throw new ConflictException("Handle already in use");throw error;});}
}