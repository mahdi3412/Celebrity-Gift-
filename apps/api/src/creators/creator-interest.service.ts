...
export class CreatorInterestService{
 ...
 async register(fanId:string,input:{handle:string;displayName:string;category?:string}){
  ...
  return this.db.transaction(async client=>{
   ...
   const metrics=(await client.query("SELECT COUNT(DISTINCT fan_id)::int AS \"uniqueFans\",COUNT(*)::int AS \"totalRequests\" FROM creator_interest_events WHERE creator_target_id=$1",[target.id])).rows[0];
 ...
 async list(search=""){
  const result=await this.db.query("SELECT t.id,t.handle,t.display_name AS \"displayName\",t.category,COUNT(DISTINCT e.fan_id)::int AS \"uniqueFans\",COUNT(e.id)::int AS \"totalRequests\" FROM creator_targets t LEFT JOIN creator_interest_events e ON e.creator_target_id=t.id WHERE ($1='' OR t.display_name ILIKE '%'||$1||'%' OR t.handle ILIKE '%'||$1||'%') GROUP BY t.id,t.handle,t.display_name,t.category ORDER BY \"uniqueFans\" DESC,t.display_name ASC LIMIT 50",[search.trim()]);
  const thresholds=await this.thresholds.get();
