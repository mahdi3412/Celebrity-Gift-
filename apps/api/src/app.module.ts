...
import {RiskController} from "./admin/risk.controller";
import {RedisModule,REDIS_CLIENT} from "./redis/redis.module";
@Module({
 imports:[ScheduleModule.forRoot(),ConfigModule.forRoot({isGlobal:true,validationSchema:Joi.object({NODE_ENV:Joi.string().default("development"),API_PORT:Joi.number().default(4000),DATABASE_URL:Joi.string().uri().required(),REDIS_URL:Joi.string().uri().required(),JWT_ACCESS_SECRET:Joi.string().min(32).required(),JWT_REFRESH_SECRET:Joi.string().min(32).required(),JWT_ACCESS_TTL:Joi.string().default("15m"),JWT_REFRESH_TTL:Joi.string().default("30d"),KYC_PROVIDER:Joi.string().valid("manual","hyperverge").default("manual"),KYC_ENCRYPTION_KEY:Joi.string().hex().length(64).when("NODE_ENV",{is:"production",then:Joi.required()}),KYC_PRIVATE_STORAGE_PATH:Joi.string().default(".private/kyc"),KYC_WEBHOOK_SECRET:Joi.string().min(32).when("NODE_ENV",{is:"production",then:Joi.required()}),HYPERVERGE_APP_ID:Joi.string().when("KYC_PROVIDER",{is:"hyperverge",then:Joi.required()}),HYPERVERGE_APP_KEY:Joi.string().when("KYC_PROVIDER",{is:"hyperverge",then:Joi.required()}),HYPERVERGE_LIVENESS_URL:Joi.string().uri().when("KYC_PROVIDER",{is:"hyperverge",then:Joi.required()}),SWAGGER_ENABLED:Joi.boolean().default(false)})}),RedisModule,ThrottlerModule.forRootAsync({imports:[RedisModule],inject:[ConfigService,REDIS_CLIENT],useFactory:(config:ConfigService,redis:any)=>({throttlers:[{name:"default",ttl:60000,limit:120}],storage:new RedisThrottlerStorage(redis)})}),DatabaseModule,AuditModule,NotificationsModule,StorageModule,CourierModule,AdminModule,PrivacyModule,SettingsModule,RiskModule,AuthModule,CreatorsModule,GiftsModule,VerificationModule,StationModule,ThresholdsModule],
