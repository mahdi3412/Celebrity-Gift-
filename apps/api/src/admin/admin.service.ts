...
export class AdminService{
 ...
 async setUserStatus(id:string,status:"ACTIVE"|"SUSPENDED",actorId:string){if(id===actorId)throw new ForbiddenException("An administrator cannot change their own account status");if(status==="SUSPENDED"){const activeAdmins=await this.db.query<{count:string}>("SELECT COUNT(*)::text AS count FROM users WHERE role='admin' AND status='ACTIVE' AND id<>$1",[id]);if(Number(activeAdmins.rows[0]?.count??0)===0)throw new ForbiddenException("At least one active administrator must remain");}const r=await this.db.query("UPDATE users SET status=$1,updated_at=now() WHERE id=$2 RETURNING id,status",[status,id]);if(!r.rowCount)return null;await this.audit.record({actorId,action:status==="SUSPENDED"?"ADMIN_USER_SUSPEND":"ADMIN_USER_ACTIVATE",entityType:"user",entityId:id,metadata:{status}});return r.rows[0];}
