...
DO $ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname='verification_status') THEN CREATE TYPE verification_status AS ENUM ('NOT_STARTED','SUBMITTED','AUTO_VERIFIED','MANUAL_REVIEW','FAILED','RESUBMIT'); END IF; END $;
DO $ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname='gift_status') THEN CREATE TYPE gift_status AS ENUM ('REQUESTED','RECEIVED_AT_STATION','PROCESSING','SHIPPED','DELIVERED','ACCEPTED','DECLINED','RETURNED'); END IF; END $;
...
DO $ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname='station_inspection_status') THEN CREATE TYPE station_inspection_status AS ENUM ('PENDING','CLEAR','DAMAGED','SUSPICIOUS','REQUIRES_REPACK'); END IF; END $;
